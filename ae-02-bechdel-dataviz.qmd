...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013))+
  ...
  labs(
    ...
    y = "Gross Profits in 2013", 
    title = "Movie budgets versus gross profit in 2013"
  )
```
...
ggplot(bechdel,aes(x = budget_2013, y = gross_2013, colour = binary, size = roi, alpha = 0.5)) + geom_point() + facet_wrap(~ clean_test) + labs(x = "Budget (million 2013 $)", y = "Budget (million 2013 $)", title = "Gross revenue vs budget, by Bechdel test result")
```
...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013, colour = binary, alpha = 0.5)) + geom_point() + facet_wrap(~ clean_test) + labs(x = "Budget (million 2013 $)", y = "Gross revenue (million 2013 $)", title = "Gross revenue vs. budget, by Bechdel test result") + scale_x_continuous(label = label_number(scale = 1/1000000)) + scale_y_continuous(label = label_number(scale = 1/1000000))
```
...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013, colour = binary)) + geom_point(alpha = 0.5) + facet_grid(clean_test~binary) + labs(x = "Budget (million 2013 $)", y = "Gross revenue (million 2013 $)", title = "Gross revenue vs. budget, by Bechdel test result") + scale_x_continuous(label = label_number(scale = 1/1000000)) + scale_y_continuous(label = label_number(scale = 1/1000000))

ggplot(bechdel, aes(x = budget_2013, y = gross_2013, colour = binary)) + geom_point(alpha = 0.5) + facet_grid(binary~clean_test) + labs(x = "Budget (million 2013 $)", y = "Gross revenue (million 2013 $)", title = "Gross revenue vs. budget, by Bechdel test result") + scale_x_continuous(label = label_number(scale = 1/1000000)) + scale_y_continuous(label = label_number(scale = 1/1000000))
```

This update is useful for comparing patterns across groups as it separates budget/gross into distinct panels for each combination of `clean_test` and `binary`.
That makes it easier to see whether the association looks similar or different across categories.

For this specific data set, it can also be less visually efficient than a single plot since some panels have few points, and it may become harder to compare trends at a glance because the data are split across many small panels.
...
ggplot(bechdel, aes(x = clean_test, y = roi,  colour = binary)) + geom_boxplot() + labs(x = "Bechdel test category", y = "ROI", title = "ROI by Bechdel test result", colour = "Bechdel\nresult") 
```
